...
#variavies iniciais
dados = open('xml_pai/psn_ag.xml', 'r', encoding="utf-8").read()
...
for i in range(len(links)):
    texto = f"""
    <!-- id: stuff_{i} handler:start | TCXS Project PSN STUFF | {titulos[i]}) -->
    <button id="btn{i}" class="caixa_imagem"  type="button" onclick="window.location.href='#janela_{titulos[i]}';" 
    autofocus>
    ...
    <!-- id: stuff_{i} handler:end | TCXS Project | {titulos[i]} -->
    \n"""

    gravar = open('stuffAG_pronto.txt', 'a')
    gravar.write(texto)

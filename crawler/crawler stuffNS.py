...
#variavies iniciais
dados = open('xml_pai/psn_ns.xml', 'r', encoding="utf-8").read()
...
for i in range(len(links)):
    texto = f"""
    <!-- id: stuff_{i + 376} handler:start | TCXS Project PSN STUFF | {titulos[i]}) -->
    <button id="btn{i + 376}" class="caixa_imagem"  type="button" onclick="window.location.href='#janela_{titulos[i]}';" 
    autofocus>
    ...
    <!-- id: stuff_{i + 376} handler:end | TCXS Project | {titulos[i]} -->
    \n"""

    gravar = open('stuffNS_pronto.txt', 'a')
    gravar.write(texto)

...
#variavies iniciais
dados = open('xml_pai/psn_hm.xml', 'r', encoding="utf-8").read()
...
for i in range(len(links)):
    texto = f"""
    <!-- id: stuff_{i+266} handler:start | TCXS Project PSN STUFF | {titulos[i]}) -->
    <button id="btn{i+266}" class="caixa_imagem"  type="button" onclick="window.location.href='#janela_{titulos[i]}';" 
    autofocus>
    ...
    <!-- id: stuff_{i+266} handler:end | TCXS Project | {titulos[i]} -->
    \n"""

    gravar = open('stuffHM_pronto.txt', 'a')
    gravar.write(texto)
